...
from dataset import Saliency, Duration
...
def main():
	global args, best_prec1, model, train_loader, val_loader, output, target
	...
	train_loader = torch.utils.data.DataLoader(
		Duration(CONFIG, 'train'), batch_size=args.batch_size, shuffle=False,
		num_workers=args.workers, pin_memory=True, sampler=None)

	val_loader = torch.utils.data.DataLoader(
		Duration( CONFIG, 'test'),
...
def train(train_loader, model, criterion, optimizer, epoch):
	...
	for i, (input, target) in enumerate(train_loader):
		...
		acc_1  = accuracy(output.data.cpu().numpy(), target.cpu().numpy())
		# acc_2  = accuracy(fov.data.cpu().numpy(), mask.cpu().numpy())

		losses.update(loss.item(), input.size(0))
		top1.update(acc_1.mean(), input.size(0))
		...
		if i % args.print_freq == 0:
			logging.info('Epoch: [{0}][{1}/{2}]\t'
				...
				# 'Fov@1 {top1_fov.val:.3f} ({top1_fov.avg:.3f})\t'.format(
