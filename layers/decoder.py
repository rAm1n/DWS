...
def make_decoder(name):
	return Custom_ConvLstm(d_config[name])
...
class AttnConvLSTMCell(nn.Module):
	...
	def __init__(self, input_size, input_dim, hidden_dim, kernel_size, bias):
		...
		super(AttnConvLSTMCell, self).__init__()
...
class Custom_ConvLstm(nn.Module):
	...
	def forward(self, input, hidden_c=None):
		...
		for t in range(output.size(1)):
			...
			# conv_output.append(self.regr(conv1_1_out.view(b, -1)))
